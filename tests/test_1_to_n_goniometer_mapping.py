...
def test_1_to_n_goniometer_mapping(tmp_path):
    ...
    with patch.dict("subhkl.instrument.goniometer.reduction_settings", {"CG4D": mock_settings}):
        ...
        expected_names = [
            "CG4D:Mot:phi", "CG4D:Mot:alpha", "CG4D:Mot:kappa", 
            "CG4D:Mot:alpha", "CG4D:Mot:omega"
        ]
        assert gonio.names_raw == expected_names, f"Suffix stripping failed. Got {gonio.names_raw}"
    ...
    with h5py.File(peaks_h5, "w") as f:
        ...
        f.create_dataset("goniometer/names", data=[n.encode('utf-8') for n in gonio.names_raw])
    ...
    with (
        ...
        mock_get_rot.return_value = (gonio.axes_raw, np.array(gonio.angles_raw)[None, :], gonio.names_raw)
        ...
        indexer(
            ...
            population_size=200, gens=50, n_runs=1, seed=42,
            refine_goniometer=True,  # <-- Crucial! Triggers the JAX motor_map fusion
            instrument_name="CG4D", original_nexus_filename=str(dummy_nexus),
        )
    ...
    with h5py.File(output_h5, "r") as f:
        ...
        assert flags["refine_goniometer"] is True, "Indexer ignored goniometer refinement flag."
