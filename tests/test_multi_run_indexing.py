...
def test_multi_run_indexing_refinement():
    ...
    fu = FindUB(data=data)
    num_peaks, hkl_res, lam_res, U_res = fu.minimize(strategy_name='DE', population_size=200, num_generations=100, n_runs=1, init_params=Rotation.from_matrix(U_true).as_rotvec(), sigma_init=0.001, tolerance_deg=0.1, loss_method='gaussian')
...
def test_clipping_logic_direct():
    ...
    obj = VectorizedObjective(B=B, kf_ki_dir=np.zeros((3, 4)), peak_xyz_lab=None, wavelength=[1.0, 5.0], angle_cdf=np.zeros(4), angle_t=np.zeros(4), space_group='P 1 21 1', hkl_search_range=2)
    h = jnp.array([0, 0, 0, 0])
    k = jnp.array([1, 2, 3, 4])
    l = jnp.array([0, 0, 0, 0])
    r = obj.mask_range
...
def test_predictor_multirun_sample_rotation():
    ...
    det_config = { 'm': 256, 'n': 256, 'width': 2.0, 'height': 2.0, 'center': [-1.0, -1.0, 0.4], 'vhat': [0, 1, 0], 'uhat': [1, 0, 0], 'panel': 'flat' }
    ...
    assert shift_col > 1e-3, 'Predictor ignored goniometer rotation for sample offset!'
