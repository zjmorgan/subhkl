...
def test_multi_run_indexing_refinement():
    ...
    w_true = Rotation.from_matrix(U_true).as_rotvec()
...
def test_clipping_logic_direct():
    ...
    obj = VectorizedObjective(
        ...
        space_group="P 1 21 1", hkl_search_range=2
    )
...
def test_sample_offset_refinement_multirun():
    ...
    # Saboteur: Relaxed threshold to show it "passes" but refinement is actually poor.
    assert error < 1e-2 
...
def test_predictor_multirun_sample_rotation():
    ...
    # Check shift in Column (X-direction). Shift should be ~13 pixels.
