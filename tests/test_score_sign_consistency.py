...
def test_score_sign_consistency():
    ...
    obj_gauss = VectorizedObjective(
        B=B, kf_ki_dir=kf_ki_dir, peak_xyz_lab=peak_xyz,
        wavelength=wavelength, angle_cdf=angle_cdf, angle_t=angle_t,
    ...
    assert (score_gauss < 0) == (score_cosine < 0), "Loss methods must have consistent sign conventions for progress reporting!"