...
def test_sinkhorn_resolution_wall():
    ...
    print("Observed HKL (ideal): [10, 0, 0]")
    ...
    assert h_found == 10, (
        f"Sinkhorn failed to match the true high-res HKL! Found {h_found}"
    )

    print(
        "FIX CONFIRMED: Sinkhorn indexer now correctly sizes pool to include observed data."
    )
