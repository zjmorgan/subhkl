...
def test_integration_worker_handles_numpy_centers():
    ...
    assert isinstance(adjusted_centers, np.ndarray), "adjusted_centers must be a numpy array"
    ...
    # i, j, h, k, l, wl
    peaks = (np.array([10]), np.array([10]), np.array([1]), np.array([0]), np.array([0]), np.array([1.0]))
    integration_params = {
        "peak_center_box_size": 3, "peak_smoothing_window_size": 3, 
    ...
    metrics_info = (None, None, None, 0, np.eye(3), np.zeros(3), np.eye(3), np.zeros(3), np.array([0,0,1]))
    
    # This should NOT raise TypeError
    res = _integrate_single_bank(0, 0, intensity, peaks, det_config, integration_params, "free_fit", viz_info, metrics_info)
